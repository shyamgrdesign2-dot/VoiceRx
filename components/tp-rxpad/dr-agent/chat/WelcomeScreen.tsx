...
export function WelcomeScreen({
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-[12px] py-[16px] relative">
      ...
      <p
        className="relative z-[1] mt-[4px] max-w-full px-1 text-[16px] text-center leading-[22px] whitespace-nowrap text-ellipsis overflow-hidden"
      ...
      <div className="relative z-[1] mt-[16px] grid grid-cols-2 gap-[10px] w-full">
        {actions.map((action, i) => (
          ...
          >
            ...
            {/* Title — 12px semibold */}
            <span className="relative z-[1] text-[12px] font-semibold leading-[16px] w-full" style={{ color: "var(--tp-slate-700, #454551)" }}>
              {action.title}
            </span>

            {/* Subtitle — 12px with 2-line clamp */}
            <span className="relative z-[1] mt-[4px] text-[12px] font-normal leading-[16px] w-full welcome-card-subtitle" style={{ color: "var(--tp-slate-400, #A2A2A8)" }}>
              {action.subtitle}
            </span>
          </button>
        ))}
