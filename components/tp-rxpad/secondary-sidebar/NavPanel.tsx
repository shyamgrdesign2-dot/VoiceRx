    ...
    >
      ...
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .tp-nav-pill-wobble,
          .tp-nav-unseen-dot { animation: none; }
        }
      `}</style>
    </div>
  )
...
function MedicalNavIcon({ active, name, hasUnseen }: { active: boolean; name: string; hasUnseen?: boolean }) {
  return (
    <IconPill active={active} hasUnseen={hasUnseen}>
      <TPMedicalIcon
        name={name}
  ...
  return (
    ...
    >
      <div className="content-stretch flex items-center relative w-full">
        ...
        >
          {icon.kind === "medical" ? (
            <MedicalNavIcon active={active} name={icon.name} hasUnseen={hasUnseen} />
          ) : (
            <IconsaxNavIcon active={active} Icon={icon.Icon} hasUnseen={hasUnseen} />
          )}
          <p
