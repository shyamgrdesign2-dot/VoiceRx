      ...
      <div className="flex flex-wrap gap-1">
        {actions.map((action) => (
          <button
            key={action}
            type="button"
            className="rounded-full border border-tp-violet-200 bg-tp-violet-50 px-1.5 py-0.5 text-[10px] font-semibold text-tp-violet-600"
          >
            {action}
          </button>
        ))}
    ...
    <div className="content-stretch flex size-full flex-col">
      <div className="border-b border-tp-slate-300 px-[10px] py-[8px]">
        <div className="mb-[7px] flex items-center gap-[6px]">
          <AiBrandSparkIcon size={20} withBackground />
          <p className="text-[12px] font-semibold text-tp-slate-700">Doctor Agent</p>
        </div>
        <div className="relative">
          <select
            value={selectedPatientId}
            onChange={(event) => setSelectedPatientId(event.target.value)}
            className="h-[30px] w-full appearance-none rounded-[8px] border border-tp-slate-200 bg-white px-[8px] pr-[26px] text-[12px] font-medium text-tp-slate-700 outline-none focus:border-tp-blue-300"
          >
      ...
      <div ref={scrollRef} className="flex-[1_0_0] space-y-[8px] overflow-y-auto p-[10px]">
        <div className="rounded-[10px] border border-tp-violet-100 bg-white p-[8px] shadow-[0_8px_16px_-14px_rgba(103,58,172,0.55)]">
          ...
          <p className="text-[10px] leading-[16px] text-tp-slate-600">
          ...
          return (
            <div key={message.id} className="space-y-[5px]">
              <div className={cn("flex", isUser ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    "max-w-[95%] rounded-[10px] px-[8px] py-[7px] text-[10px] leading-[16px]",
                    isUser
      ...
      <div className="shrink-0 border-t border-tp-slate-300 p-[10px]">
        <div className="mb-[7px] flex flex-wrap gap-[4px]">
          {QUICK_PROMPTS.map((prompt) => (
            <button
              ...
              className="rounded-full border border-tp-blue-200 bg-tp-blue-50 px-[7px] py-[3px] text-[10px] font-semibold text-tp-blue-500"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="flex items-center gap-[6px]">
          <input
            ...
            className="h-[30px] min-w-0 flex-1 rounded-[8px] border border-tp-slate-200 bg-tp-slate-50 px-[10px] text-[12px] text-tp-slate-700 outline-none focus:border-tp-blue-300"
          />
          <button
            type="button"
