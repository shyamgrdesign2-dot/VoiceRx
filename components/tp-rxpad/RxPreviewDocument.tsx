...
function SectionList({ title, rows }: { title: string; rows: RxPreviewLine[] }) {
  ...
  return (
    <section className="flex flex-col gap-[4px]">
      <h3 className="text-[13px] font-semibold leading-[18px] text-tp-slate-900">{title}</h3>
      <ul className="m-0 flex list-disc flex-col gap-[4px] pl-[18px] marker:text-tp-slate-500">
        {rows.map((row, i) => (
          <li key={`${title}-${i}`} className="text-[11px] leading-[16px] text-tp-slate-700">
        ...
        <header className="mb-[14px] rounded-[6px] bg-tp-slate-100/60 px-[10px] py-[8px]">
          <div className="flex items-start justify-between gap-[12px]">
            <div className="flex items-start gap-[12px]">
              ...
              <div>
                <p className="text-[15px] font-semibold leading-[20px] text-tp-blue-700">TP Clinic</p>
                <p className="text-[11px] font-medium leading-[16px] text-tp-slate-700">
        ...
        <section className="py-[8px]">
          <div className="flex items-start justify-between gap-[24px]">
            <div className="flex min-w-0 flex-1 flex-col gap-[2px]">
              <p className="text-[11px] leading-[16px] text-tp-slate-700">
        ...
        <div className="flex-1 overflow-hidden">
          {!snapshot ? (
            <div className="py-[20px]">
              <p className="text-[11px] leading-[16px] text-tp-slate-600">
            ...
            <div className="flex flex-col gap-[10px] py-[8px]">
              ...
              {snapshot.followUp ? (
                <section className="flex flex-col gap-[2px]">
                  <h3 className="flex items-center gap-[5px] text-[13px] font-semibold leading-[18px] text-tp-slate-900">
                    <Calendar2 size={14} color="var(--tp-slate-500)" variant="Bulk" />
                    Follow Up
                  </h3>
                  <p className="text-[11px] leading-[16px] text-tp-slate-700">{snapshot.followUp}</p>
                </section>
              ) : null}
              {snapshot.additionalNotes ? (
                <section className="flex flex-col gap-[2px]">
                  ...
                  <p className="text-[11px] leading-[16px] text-tp-slate-700">{snapshot.additionalNotes}</p>
                </section>
              ) : null}
            </div>
          )}
