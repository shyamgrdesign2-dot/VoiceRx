    ...
    >
      {title && (
        <div className="px-3 py-1.5 mb-0.5">
          <span className="text-[12px] font-bold uppercase tracking-wider text-tp-slate-400">
            {title}
          </span>
        </div>
      )}
      {items.map((item, i) =>
        item.separator ? (
          ...
          >
            ...
            {item.shortcut && (
              <span className="text-[12px] font-mono text-tp-slate-400 flex items-center gap-0.5">
                {item.shortcut}
              </span>
            )}
