    ...
    <details open className="group/visit mb-3 last:mb-0">
      ...
      ">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-tp-slate-500 mt-0.5">{visit.doctorName}{visit.speciality ? ` — ${visit.speciality}` : ""}</p>
        </div>
        <CopyButton
      ...
      <div className="rounded-b-[10px] border border-t-0 border-tp-slate-200 bg-white px-3 py-2 space-y-3">
        ...
        {visit.advices && visit.advices.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-tp-slate-500">Advices</span>
              <CopySectionButton
                label="Copy All"
                onCopy={() => onCopyToRxPad?.({
                  target: "advices",
        ...
        {/* Notes */}
        {visit.notes && (
          <p className="text-[11px] italic text-tp-slate-500 border-t border-tp-slate-100 pt-2">{visit.notes}</p>
        )}
      </div>
    </details>
  )
