  ...
  return (
    ...
    >
      <div ref={moduleRootRef} data-rx-module-root="true" className="space-y-[18px]">
        ...
        {voiceActive ? (
          // Kept inside the section's p-[18px] padding so the gradient
          // card breathes on left / right / bottom rather than stretching
          // edge-to-edge. Rounded on all four corners now (the card no
          // longer has to continue the module card's rounded-bottom).
          <div>
            <VoiceRxModuleRecorder
              sectionLabel={title}
              onCancel={() => onVoiceClick?.()}
              onSubmit={() => onVoiceClick?.()}
              radiusClassName="rounded-[14px]"
            />
          </div>
        ) : null}
        <div className={`relative ${voiceActive ? "hidden" : ""}`}>
          <TPRxPadSearchInput
            ref={searchInputRef}
