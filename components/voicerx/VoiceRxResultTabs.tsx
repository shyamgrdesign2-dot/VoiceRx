...
import { ConversationTranscript, DictationTranscript } from "./VoiceTranscriptProcessingCard"
...
type AudioQuality = "good" | "fair" | "poor"
        ...
        <span className="vrx-rt-mode-pill inline-flex items-center gap-[7px] rounded-[10px] py-[3px] pl-[5px] pr-[4px]">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-[200px] border-tp-slate-200 bg-white">
              <div className="px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-tp-slate-400">
                Current Session: VRX-8149
              </div>
              <DropdownMenuSeparator className="bg-tp-slate-100" />
              <div className="px-2 py-1 text-[11.5px] font-semibold text-tp-slate-500">
                Session History
              </div>
              <DropdownMenuItem className="text-[13px] text-tp-slate-700 hover:bg-tp-slate-50 flex items-center justify-between">
                <span>12 Oct, Consultation</span>
                <span className="text-[11px] text-tp-slate-400">View</span>
              </DropdownMenuItem>
              <DropdownMenuItem className="text-[13px] text-tp-slate-700 hover:bg-tp-slate-50 flex items-center justify-between">
              ...
              <DropdownMenuSeparator className="bg-tp-slate-100" />
              <DropdownMenuItem className="text-[13px] text-tp-slate-700 hover:bg-tp-slate-50">
                Settings
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
...
function TranscriptPane({ transcript, audioQuality: _audioQuality }: { transcript: string; audioQuality: AudioQuality }) {
  // Detect Doctor:/Patient: markers — if present, render the same
  // bubble UI used by the active-agent shiner card so the doctor sees
  // the structured conversation here too. Otherwise fall back to a
  // plain dictation paragraph.
  const isConversation = /\b(Doctor|Patient)\s*:/i.test(transcript)
  if (!transcript) {
    return (
  ...
  return (
    <div className="flex flex-col gap-3 pt-1">
      {/* Compact UI-card-in-UI-card framing — same calm slate-100 the
          Dr. Agent chat uses behind its transcript bubbles, so the
          white doctor bubble + violet patient bubble read crisply on
          top. The surrounding panel carries the rotating violet wash
          backdrop; this slate frame is the contrast surface that
          makes the bubbles legible. */}
...
function EmrPaneFallback({ sections }: { sections: EmrSection[] }) {
  if (!sections.length) {
    return <div className="rounded-[12px] border border-dashed border-tp-slate-200 bg-tp-slate-50/40 px-4 py-6 text-center text-[13px] text-tp-slate-500">No EMR data yet.</div>
  }
  return (
      ...
      {sections.map((s) => (
        <div key={s.id} className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <h4 className="text-[13.5px] font-bold uppercase tracking-[0.06em] text-tp-slate-800">{s.title}</h4>
          </div>
          {s.items.length ? (
            ...
            <p className="text-[13.5px] italic text-tp-slate-400">No content</p>
          )}
        </div>
      ))}
    </div>
...
function FeedbackRow({ value, onChange, audioQuality }: { value: Feedback; onChange: (v: Feedback) => void; audioQuality?: AudioQuality }) {
  return (
    <div className="mt-3 flex items-center gap-1.5">
      ...
      {audioQuality && (
        <>
          ...
          <div className="flex items-center gap-1 text-[13.5px] font-medium text-emerald-600">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="top" sideOffset={6} className="max-w-[220px] rounded-[6px] border-0 bg-tp-slate-900 px-2.5 py-1.5 text-[11.5px] leading-[1.45] text-white shadow-[0_8px_20px_-10px_rgba(15,23,42,0.45)]">
...
function StickyFooter({
  ...
  const tooltipDarkCls = "rounded-[6px] border-0 bg-tp-slate-900 px-2.5 py-1.5 text-[11.5px] leading-[1.45] text-white shadow-[0_8px_20px_-10px_rgba(15,23,42,0.45)]"
  const primaryHelp =
        ...
        {isClinicalCopy ? (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="center" className="w-[220px] border-tp-slate-200 bg-white rounded-[10px] p-1">
              <DropdownMenuItem onClick={onCopyClipboard} className="text-[13.5px] text-tp-slate-700 py-2 rounded-[6px]">
                <CopyIcon size={15} /> <span className="ml-2">Copy to Clipboard</span>
              </DropdownMenuItem>
              {onCopyToRx && (
                <DropdownMenuItem onClick={onCopyToRx} className="text-[13.5px] text-tp-slate-700 py-2 rounded-[6px]">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  onClick={onPrint}
                  aria-label="Print"
                  className="vrx-rt-secondary-blue flex h-[42px] flex-1 items-center justify-center gap-2 rounded-[10px] transition-colors shrink-0 text-[13.5px] font-semibold"
                >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                onClick={onAddVoice}
                disabled={!onAddVoice}
                className="vrx-rt-voice-cta-outline flex h-[42px] flex-1 items-center justify-center gap-2 rounded-[10px] px-3 text-[13.5px] font-semibold transition-transform active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-50"
              >
