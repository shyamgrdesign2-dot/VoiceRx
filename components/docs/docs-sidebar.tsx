    ...
    <nav className="flex h-full flex-col" aria-label="Documentation">
      ...
      <div className="shrink-0 px-5 pt-6 pb-4">
        <Link href="/" className="block" onClick={onNavigate}>
          ...
          <p className="mt-0.5 text-[16px] font-bold text-tp-slate-900 tracking-tight">
            Design System
          </p>
        </Link>
      </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={onNavigate}
                    className={`group flex items-center gap-2.5 rounded-lg px-3 py-2 text-[14px] font-medium transition-colors ${
                      isActive
                        ? "bg-tp-blue-50 text-tp-blue-700"
                        : "text-tp-slate-600 hover:bg-tp-slate-50 hover:text-tp-slate-900"
                    }`}
